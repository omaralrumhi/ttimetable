/**
 * نظام الجدول المدرسي الذكي - نسخة منع كسر الكلمات
 */
...
function updateUI(rowsSun, rowsToday, dayName, phraseText) {
    ...
    for (let i = 2; i < rowsSun.length; i++) {
        ...
        if (isActive) {
            ...
            currentData = { title: "الحصة الحالية: " + sRow[0], end: sRow[2], html: cards };
        }
    }
...
function adjustIndividualCardFonts() {
    ...
    cards.forEach(card => {
        ...
        // منع كسر الكلمات والنزول لسطر جديد نهائياً
        wrapper.style.whiteSpace = "nowrap"; 
        wrapper.style.wordBreak = "keep-all";
        wrapper.style.display = "inline-block";

        // تصغير الخط حتى يختفي التمرير الأفقي (scrollWidth) ويصبح النص داخل عرض الكرت

/**
 * نظام إدارة جدول الحصص الذكي - النسخة الذهبية
 * مع شريط متحرك ديناميكي وتصغير خط ذكي
 */
...
async function initSystem() {
    ...
    try {
        ...
        try {
            ...
            // تكرار العبارات لضمان عدم وجود فراغ كبير في الشريط
...
function updateUI(rowsSun, rowsToday, dayName, phraseText) {
    ...
    if (currentData) {
        ...
        list.innerHTML = currentData.html;
        
        // تشغيل الوظائف الذكية
        setTimeout(() => {
            adjustTeacherFontSizes();
        }, 100);
...
function adjustTeacherFontSizes() {
    ...
    names.forEach(el => {
        ...
        // فحص العرض والارتفاع لضمان بقاء الاسم داخل الكرت تماماً
        while ((el.scrollWidth > parent.clientWidth - 10 || el.scrollHeight > parent.clientHeight - 20) && size > 0.6) {
            size -= 0.05;
            el.style.fontSize = size + 'vw';
        }
    });
...
setInterval(() => {
    ...
    document.getElementById('miladi-date').innerText = now.toLocaleDateString('ar-EG-u-nu-latn', options);
}, 1000);

// إعادة ضبط الخطوط عند تغيير حجم النافذة (الزووم)
window.addEventListener('resize', adjustTeacherFontSizes);
